...
def run_h3dnet_with_pca_cv(
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(X_full, y_full), 1):
        ...
        # --- PCA fit on training spectra only ---
        Xtrain_flat = Xtr_raw.reshape(-1, B)
        pca = PCA(n_components=min(max_components, B))
        Xtrain_pca = pca.fit_transform(Xtrain_flat)

        cumvar = np.cumsum(pca.explained_variance_ratio_)
        D = min(np.argmax(cumvar >= var_thresh) + 1, max_components)

        explained = pca.explained_variance_ratio_[:D]
        pca_components_list.append(int(D))
        pca_var_list.append(explained.tolist())

        print(f"[Fold {fold}] PCA comps: {D} ({cumvar[D-1]*100:.2f}% variance)")

        # Apply PCA to val
        Xval_flat = Xval_raw.reshape(-1, B)
        Xval_pca = pca.transform(Xval_flat)

        # Reshape
        Xtr = Xtrain_pca[:, :D].reshape(Ntr, H, W, D, 1)
        Xval = Xval_pca[:, :D].reshape(Xval_raw.shape[0], H, W, D, 1)
        ...
        fold_histories.append({k: list(v) for k, v in history.history.items()})
    ...
    print(f"Histories saved: {out_hist}")
