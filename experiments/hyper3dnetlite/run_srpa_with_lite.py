...
from hyperspectral_insight.band_selection.srpa import srpa_selection_3dcnn
...
def run_srpa(
    ...
    rho: float = 0.95,
    epsilon: float = 1e-7,
    save_dir: str = "results/hyper3dnetlite/srpa/new/",
    verbose: bool = True,
    max_samples_per_class: int = None,
):
    """
    Hyper3DNet-Lite with SRPA band selection.

    Pipeline:
        1) Load dataset
        2) Normalize cube per band
        3) Run SRPA selection (supervised)
    ...
    print(f"\n=== Hyper3DNet-Lite + SRPA ({num_bands} bands) on {dataset_name} ===")
    ...
    out_json = os.path.join(
        save_dir,
        f"{dataset_name}_lite_srpa_{num_bands}bands_{n_splits}fold_cv.json",
    )
    out_bands = os.path.join(
        save_dir,
        f"{dataset_name}_lite_srpa_{num_bands}bands_{n_splits}fold_cv.npy",
    )
